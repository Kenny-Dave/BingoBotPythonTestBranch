...
# it goes through the words and keeps reducing the size so each word fits in the box.
def longWord(text):
...
# takes the text and fontsize from longWord, and reduces the text size further so that the string will fit in the box,
            ...
            if font.getsize(line)[0] <= boxImTestW:
                ...
                for word in line.split(" "):
                    ...
                    if font.getsize(current_line + word)[0] <= boxImTestW:  # if the next word fits on the line
                        if current_line != "":
                            current_line += " " + word  # if there's already a word on the line
                        else:
                            current_line = word  # for the first word on the line
                    else:
                        if true_lines == "":
                            true_lines = current_line  # for the first line
                        else:
                            true_lines = true_lines + "\n" + current_line  # adds subsequent lines
                        current_line = word  # putting the current word on the next line

                if true_lines == "":
                    true_lines = current_line  # writing the first line to true_lines
                else:
                    true_lines = true_lines + "\n" + current_line  # writing subsequent lines to true lines

        w, h = image_draw.textsize(true_lines, font)

        if (w <= boxImTestW and h <= boxImTestH) or font.size == 6:
            # if it fits, or you're down to font size 6, return true lines and the font
